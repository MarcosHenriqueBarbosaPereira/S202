...
cassandra_session.set_keyspace('ksiot') # Change to your keyspace

redis_conn = redis.Redis(
    host="redis-18230.c263.us-east-1-2.ec2.redns.redis-cloud.com", port=18230, #Use your Redis instance host and port
...
def questao_1_a(users):
    ...
    query = "DROP TABLE IF EXISTS venda"
    cassandra_session.execute(query)

    query = "CREATE TABLE IF NOT EXISTS usuario (id int, estado text, cidade text, endereco text, nome text, email text, interesses list<text>, PRIMARY KEY ((estado, cidade), id))"
    cassandra_session.execute(query)

    query = "CREATE TABLE IF NOT EXISTS produto (id int, categoria text, nome text, custo int, preco int, quantidade int, PRIMARY KEY ((categoria), id))"
    cassandra_session.execute(query)

    query = "CREATE TABLE IF NOT EXISTS venda (id int, dia int, mes int, ano int, hora text, valor int, produtos frozen<list<map<int, int>>>, usuario map<text, text>, PRIMARY KEY ((dia, mes, ano), hora, id))"
    cassandra_session.execute(query)

    for user in users:
        query = f"INSERT INTO usuario (id, estado, cidade, endereco, nome, email, interesses) VALUES ({user['id']}, '{user['estado']}', '{user['cidade']}', '{user['endereco']}', '{user['nome']}', '{user['email']}', {user['interesses']})"
        cassandra_session.execute(query)

    users = cassandra_session.execute("SELECT * FROM usuario")
...
def questao_1_b(products):
    ...
    for product in products:
        query = f"INSERT INTO produto (id, categoria, nome, custo, preco, quantidade) VALUES ({product['id']}, '{product['categoria']}', '{product['nome']}', {int(product['custo']*100)}, {int(product['preco']*100)}, {product['quantidade']})"
        cassandra_session.execute(query)

    stocks = cassandra_session.execute("SELECT * FROM produto")

    total_cost = 0

    for stock in stocks:
        total_cost += float((stock['custo'] * stock['quantidade'])/100.0)
...
def questao_2(state):
    ...
    users = cassandra_session.execute(f"SELECT * FROM usuario WHERE estado = '{state}' ALLOW FILTERING")

    for user in users:
        redis_conn.hmset(f"usuario:{user['id']}", mapping={
            "id": user['id'],
            "estado": user['estado'],
            "cidade": user['cidade'],
        ...
        for interest in user['interesses']:
            redis_conn.rpush(f"user_interest:{user['id']}:interesses", interest)

        result.append(redis_conn.hgetall(f"usuario:{user['id']}"))
        result2.append(redis_conn.lrange(f"user_interest:{user['id']}:interesses", 0, -1))

    #Pareia as duas listas
    for i in range(len(result)):
        result[i]["interesses"] = result2[i]
...
def questao_3(user_id):
    ...
    user_interests = redis_conn.lrange(f"user_interest:{user_id}:interesses", 0, -1)
